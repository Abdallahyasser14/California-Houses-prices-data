...
class linear_regression_gradient_descent_lasso:
    ...
    def fit(self,X,y):
        ...
        for iteration in range(self.iterations):
            ...
            loss = ((1 / (2 * X.shape[0])) * np.sum(error**2))+ (self.lambda_param/( X.shape[0]))*np.sum(np.abs(weights))
            grad = (1 / X.shape[0]) * (x_matrix.T @ error)
            reg = (self.lambda_param / X.shape[0]) * np.sign(self.parameters)
            reg[0] = 0   
